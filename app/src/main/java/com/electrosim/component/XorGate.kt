...
class XorGate(
    ...
    init {
        when (portcountinput) {
            2 -> {
                initiailizeElement(BitmapFactory.decodeResource(res, R.drawable.ugate1), x, y)
                addInputPort(InputPort(-200f, -50f, this))
                addInputPort(InputPort(-200f, 50f, this))
                addOutputPort(OutputPort(200f, 0f, this))
            }
            4 -> {
                initiailizeElement(BitmapFactory.decodeResource(res, R.drawable.ufourgate1), x, y)
    ...
    override fun simulate() {
        if (defValue == null) {
            // XOR has no initial value until first input
            for (ip in inputPorts) {
                ...
                // If defValue is null, initialize with first input, otherwise XOR with it
                defValue = if (defValue == null) invalue else defValue!! xor invalue
            }
        }
